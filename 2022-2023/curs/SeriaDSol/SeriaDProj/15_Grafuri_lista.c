...
Nod* push(Nod* list, unsigned int varf)
{
...
Nod* pop(Nod* list, unsigned int* varf_extras)
{
	if (list)
	{
		*varf_extras = list->idVarf;
...
void main() {
	...
	while (fgets(buffer, sizeof(buffer), f)) {
		...
		if (tmpLP) {
			// nodul cu idVarf == src si idNodAdiacent == dst
			tmpLP->primListaVecini = inserareNodLS(tmpLP->primListaVecini, dst);

			// stocare muchie simetrica
	...
	// apel functie de traversare
	unsigned int* OutStream = DF(pListaAdiacenta, 3, nrVarfuri);
	printf("\n\nOrdine varfuri la traversarea de tip Depth-First: ");
	for (unsigned char i = 0; i < nrVarfuri; i++)
		printf(" %u", OutStream[i]);

	// dezalocare vector cu secventa de noduri traversate DF
