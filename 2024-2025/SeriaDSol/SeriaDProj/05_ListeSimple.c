...
struct Student stergereStudent(struct Nod** p) {
	struct Nod* tmp = *p; // *p este continut de la adresa stocata in parametrul p
	struct Student s;
	if (tmp) {
...
int main() {
	...
	// dezalocare nume student extras din lista simpla
	free(stud.nume);

	// dezalocare structura lista simpla
	while (prim != NULL) // stergere repetata a ultimul nod pana cand lista devine empty
	{
		stud = stergereStudent(&prim);
		free(stud.nume);
	}
